...
$json = json_decode(file_get_contents("https://anigram.uz/api/v1?key=$api&action=add&service=$servis&link=$url&quantity=$son"),true);
$order = $json['order'];
bot('editMessageText',[
        'chat_id'=>$cid2,
        'message_id'=>$mid2,
...
if($step == "1"){
if(is_numeric($text)==true){
$orderstat=json_decode(file_get_contents("https://anigram.uz/api/v1?key=$api&action=status&order=$text"),true);
...
if($data == "balans"){
$json = json_decode(file_get_contents("https://anigram.uz/api/v1?key=$api&action=balance"),true);
